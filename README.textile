...
* Compound/Conformer (Compound and Conformer are considered to be distinct objects in this context, i.e. there is no inheritance involved. A compound is, inter alia, characterized by its conformers)
* Feature
...
Each component needs a method to be transformed into an Ontological Model (OntModel) and of course a method for the inverse process (Parsing a given Ontological Model into a component). These methods can be bundled in an Interface called RDFable: <tt>OntModel createOntModel()</tt> and <tt>void createFromRDF(OntModel)</tt>. The use of parsing utilities will facilitate the construction of components from remote resources or files. Where necessary, there should be a list of formal data types for an OpenTox component, refering to an ontological class for which the component into consideration is an instance. For example, a feature can be an instance of ot:NumericFeature etc.

h4. 1.2.1. Compound/Conformer:

* Chemical Identifiers: CasRN, Smiles and other related properties are retrieved on request and not upon construction.
* A compound has a list of conformers that are updated upon request.
...
* Set of ontological classes formally classifying the algorithm (according to the OpenTox algorithms ontology; see http://opentox.org/dev/apis/api-1.1/Algorithms
...
* Task status (Completed, Error, Cancelled)
* Percentage Completed
* Result URI (null if not completed)
* Error Report (In case of exceptional events)

h3. Make it available online

The following OpenTox compounds are POSTable by the client, i.e. a client can perform a POST request on a service and obtain a URI for its resource. Then it will be available online on some related server. This way clients can publish datasets, compounds and features.
...
Upon such a POST request, servers either return a Task or the URI of the created resource.
...
* Jena, version 2.6.2: Download Jena from http://sourceforge.net/projects/jena/files/Jena/Jena-2.6.2/jena-2.6.2.zip/download. [Make sure to download version 2.6.2. otherwise there is no guarantee it will work as expected.]
