...
public class Algorithm extends OTOnlineResource<Algorithm>
    ...
    public HTMLContainer inHtml() {
        ...
        builder.addParagraph("Specify the dataset that will be used for the training and the prediction feature URI from this dataset."
                + "Provide your preferred feature service (it is suggested to use the default one). ", Alignment.justify,"padding-left:20px;");
        HTMLForm form = builder.addForm("/iface/generic", "POST","multipart/form-data").setStyle("margin-left:20px;");
        form.addComponent(new HTMLInputImpl().setType(HTMLInput.HTMLInputType.HIDDEN).
                setName("comingFrom").setValue("webInterface"));
        form.addComponent(new HTMLInputImpl().setType(HTMLInput.HTMLInputType.HIDDEN).
                setName("algorithm_uri").setValue(getUri().toString()));
        ...
        if (getParameters() != null && !getParameters().isEmpty()) {
            ...
            HTMLTable parametersTable = builder.addTable(4).setStyle("margin-left:20px;");
